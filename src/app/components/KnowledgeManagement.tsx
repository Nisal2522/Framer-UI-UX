...
const DOCUMENT_TYPE_OPTIONS = uniqueSorted(materials.map((m) => m.documentType));
const TAG_OPTIONS = uniqueSorted(materials.flatMap((m) => m.tags));

type SortOption = "recent" | "views" | "downloads";
...
export function KnowledgeManagement() {
  ...
  const activeFilterCount = documentTypeFilter.length + tagFilter.length;
  ...
  const activeChips = [
    ...documentTypeFilter.map((v) => ({
      key: `doc-${v}`,
      label: v,
      onRemove: () => setDocumentTypeFilter((prev) => prev.filter((x) => x !== v)),
    })),
    ...tagFilter.map((v) => ({
      key: `tag-${v}`,
      label: v,
      onRemove: () => setTagFilter((prev) => prev.filter((x) => x !== v)),
    })),
  ];
  ...
  const filteredMaterials = useMemo(() => {
    ...
    const filtered = materials.filter((m) => {
      ...
      const matchesDocumentType =
        documentTypeFilter.length === 0 || documentTypeFilter.includes(m.documentType);
      const matchesTags =
        tagFilter.length === 0 || m.tags.some((t) => tagFilter.includes(t));

      return matchesSearch && matchesDocumentType && matchesTags;
    });
    ...
    return sorted;
  }, [searchTerm, documentTypeFilter, tagFilter, sortBy]);
    ...
    <div className="space-y-6 min-w-0 max-w-full">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white focus:ring-2 focus:ring-[#032EA1] focus:border-transparent outline-none"
        ...
        {activeChips.length > 0 && (
          <div className="mt-3 flex flex-wrap items-center gap-1.5 pt-3 border-t border-gray-100">
            {activeChips.map((chip) => (
              <span
                key={chip.key}
                className="inline-flex items-center gap-1 rounded-md border border-[#032EA1]/20 bg-[#032EA1]/10 px-2 py-1 text-xs font-medium text-[#032EA1]"
              >
                {chip.label}
                <X
                  className="w-3 h-3 cursor-pointer hover:text-[#021c5e]"
            ...
            <button
              ...
              className="text-xs font-medium text-gray-500 hover:text-gray-700 hover:underline ml-1"
            >
              Clear all
            </button>
          </div>
