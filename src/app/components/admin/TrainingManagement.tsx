...
export function TrainingManagement() {
  ...
  useEffect(() => {
    if (!formOpen) return;
    const frame = requestAnimationFrame(() => setFormVisible(true));
    return () => cancelAnimationFrame(frame);
  }, [formOpen]);

  useEffect(() => {
    if (!participantsDrawerId) return;
    const frame = requestAnimationFrame(() => setParticipantsVisible(true));
    return () => cancelAnimationFrame(frame);
  }, [participantsDrawerId]);

  useEffect(() => {
    if (!formOpen && !participantsDrawerId) return;
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        if (formOpen) closeForm();
        else if (participantsDrawerId) setParticipantsDrawerId(null);
      }
    };
    document.addEventListener("keydown", handleEscape);
    return () => document.removeEventListener("keydown", handleEscape);
  }, [formOpen, participantsDrawerId]);
  ...
  return (
    <div className="space-y-6 min-w-0 max-w-full">
                    ...
                    >
                      ...
                      <td className="px-2 sm:px-3 py-2.5 align-middle max-w-0">
                        <div className="h-1 w-3/5 rounded-full bg-gray-100 overflow-hidden">
                          <div
                            className="h-full rounded-full bg-[#032EA1] transition-all duration-300"
                            style={{ width: `${fillPct}%` }}
                          />
                        </div>
      ...
      {formOpen && (
        <>
          <div
            className={`fixed inset-0 z-[100] bg-black/40 transition-opacity duration-200 ${formVisible ? "opacity-100" : "opacity-0"}`}
            aria-hidden
            onClick={closeForm}
          />
          <div
            className={`fixed inset-y-0 right-0 z-[110] flex w-full max-w-2xl flex-col border-l border-gray-200 bg-gray-50 shadow-2xl transition-transform duration-200 ease-in-out ${formVisible ? "translate-x-0" : "translate-x-full"}`}
            role="dialog"
            aria-modal="true"
      ...
      {participantsTraining && (
        <>
          <div
            className={`fixed inset-0 z-[100] bg-black/40 transition-opacity duration-200 ${participantsVisible ? "opacity-100" : "opacity-0"}`}
            aria-hidden
            onClick={() => {
              setParticipantsVisible(false);
              setTimeout(() => setParticipantsDrawerId(null), 200);
            }}
          />
          <div
            className={`fixed inset-y-0 right-0 z-[110] flex w-full max-w-2xl flex-col border-l border-gray-200 bg-gray-50 shadow-2xl transition-transform duration-200 ease-in-out ${participantsVisible ? "translate-x-0" : "translate-x-full"}`}
            role="dialog"
            aria-modal="true"
